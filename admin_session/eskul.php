...
// AMBIL SEMUA FOTO
$stmt = mysqli_prepare($koneksi, "SELECT id, nama_eskul, deskripsi, file_foto, created_at FROM eskul ORDER BY created_at DESC");
...
<head>
    ...
    <title>Dashboard - Galeri Ekstrakulikuler</title>
...
<?php include 'sidebar.php'; ?>
    ...
    <main class="content">
        ...
        <div class="topbar">
            ...
            <a href="eskul.php" class="btn btn-primary" target="_blank">
                <i class="fas fa-eye"></i> Lihat Ekstrakurikuler
            </a>
        </div>
        ...
        <!-- Grid Foto -->
        <div class="card">
            <h2>Daftar Ekstrakulikuler (<?= count($photos) ?>)</h2>
                ...
                <div class="grid">
                    <?php foreach ($photos as $photo): ?>
                        <div class="photo-card">
                            <img src="uploads/eskul/<?= htmlspecialchars($photo['file_foto']) ?>" 
                                 alt="<?= htmlspecialchars($photo['nama_eskul']) ?>"
                                 onerror="this.src='https://via.placeholder.com/300x200?text=No+Image'">
                            <div class="body">
                                <h5><?= htmlspecialchars($photo['nama_eskul']) ?></h5>
                                ...
                                <div class="actions" style="margin-top: 12px;">
                                    ...
                                    <button class="btn btn-danger btn-sm" onclick="deletePhoto(<?= $photo['id'] ?>, '<?= htmlspecialchars(addslashes($photo['nama_eskul'])) ?>')">
    ...
    <div class="modal-bg" id="addModal">
        <div class="modal">
            ...
            <form action="upload_eskul.php" method="POST" enctype="multipart/form-data">
                <div>
                    <label>Nama Ekstrakurikuler</label>
                    <input type="text" name="nama_eskul" class="form-control" required placeholder="Contoh: Kegiatan Belajar">
                </div>
                ...
                <div>
                    <label>Upload Foto * (JPG, PNG, GIF max 5MB)</label>
                    <input type="file" name="file_foto" class="form-control" accept="image/*" required>
                    <small style="color: #666;">Format: JPG, PNG, GIF | Ukuran maksimal: 5MB</small>
                </div>
    ...
    <div class="modal-bg" id="editModal">
        <div class="modal">
            <h3>✏️ Edit Foto</h3>
            <form action="update_eskul.php" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="id" id="edit_id">
                
                <div>
                    <label>Nama Foto *</label>
                    <input type="text" name="nama_eskul" id="edit_nama" class="form-control" required>
                </div>
    ...
    <script>
        ...
        function editPhoto(photo) {
            ...
            document.getElementById('edit_nama').value = photo.nama_eskul;
            document.getElementById('edit_deskripsi').value = photo.deskripsi;
            document.getElementById('edit_preview').src = '../uploads/eskul/' + photo.file_foto;
            openModal('editModal');
        }
